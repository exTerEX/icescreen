...
def tryAddingSPToConjugaisonModuleEMStructure(
    ...
    if (SPtoTestForAdd.SPType in icescreen_OO.setIntegraseNames):  # SPtoTestForAdd.SPType == "IntTyr" or SPtoTestForAdd.SPType == "IntSer" or SPtoTestForAdd.SPType == "DDE"
    ...
    # (2) the SPtoTestForAdd is conjugaison module (Relaxase, VirB4, Couplage) && this BasicEMStructure has already at least one of this type of SP registred && the SPtoTestForAdd is not adjacent in the genome to the last SP registred
        ...
        if EMStructureSent.listCouplingProtein:  # list is not empty
            # lastSPAdded = EMStructureSent.listCouplingProtein[-1]# get the last SP added to this list
            # get the last SP added to this list that is not in conflict
            for lastSPAdded in reversed(EMStructureSent.listCouplingProtein):
                if len(lastSPAdded.setICEsIMEsStructureInConflict) != 0:  # in conflict, continue loop
                    continue
                else:
                    # if (abs(lastSPAdded.CDSPositionInGenome - SPtoTestForAdd.CDSPositionInGenome) > 2):  # SPs are not adjacent in the genome
        ...
        if EMStructureSent.listVirB4:  # list is not empty
            ...
            # get the last SP added to this list that is not in conflict
            for lastSPAdded in reversed(EMStructureSent.listVirB4):
                if len(lastSPAdded.setICEsIMEsStructureInConflict) != 0:  # in conflict, continue loop
                    continue
                else:
                    # if (abs(lastSPAdded.CDSPositionInGenome - SPtoTestForAdd.CDSPositionInGenome) != 1):  # SPs are not adjacent in the genome
                    #    if debugtryAddingSPToConjugaisonModuleEMStructure:
...
def testSPFamilyInfoCompatibilityWithICEsIMEsStructure(
    ...
    if len(ICEsIMEsStructureToTest.listOrderedSPs) == 0:  # list is empty
        ...
        return True
    elif (groupListSPintoICEsIMEsUsingFamilyInfo == "STRICT"):
    ...
    elif (groupListSPintoICEsIMEsUsingFamilyInfo == "LOOSE"):
        ...
        if SPToTest.SPDetectedByBlast == 1:
            ...
            raise RuntimeError(
                    "Error in testSPFamilyInfoCompatibilityWithICEsIMEsStructure: SPToTest.SPDetectedByBlast and SPToTest.SPDetectedByHMM are both 0 for SPToTest.locusTag = {}".format(SPToTest.locusTag))
    elif (groupListSPintoICEsIMEsUsingFamilyInfo == "NO"):  # "NO" means that the SP of the conjugaison module can be be affilitated with any family
        return True  # do nothing
    else:
        raise RuntimeError(
